...
if [ ! $# -ne 1 ]; then
	if [ "commit" = $1 ]; then
		docker commit ros2-docker-apple-silicon-docker ros2-docker-apple-silicon:latest
		CONTAINER_ID=$(docker ps -a -f name=ros2-docker-apple-silicon-docker --format "{{.ID}}")
...
if [ ! $# -ne 1 ]; then
	if [ "delete" = $1 ]; then
		...
		docker stop $CONTAINER_ID
		docker rm $CONTAINER_ID -f
		docker image rm ros2-docker-apple-silicon
		exit 0
	fi
fi
...
DOCKER_OPT=""
DOCKER_NAME="ros2-docker-apple-silicon-docker"
	...
	if [ ! $# -ne 1 ]; then
		if [ "xrdp" = $1 ]; then
			...
			docker run ${DOCKER_OPT} \
				...
				ros2-docker-apple-silicon:latest
		else
			docker run ${DOCKER_OPT} \
				--name=${DOCKER_NAME} \
